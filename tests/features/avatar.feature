Feature: Avatar — speaking overlay instructor
  ...
  Scenario: The avatar overlay character appears
    When I navigate to "/components/examples/avatar"
    And I wait for the page to be interactive
    Then the avatar overlay "gatin-avatar" is visible

  Scenario: The trigger starts the avatar speaking
    When I navigate to "/components/examples/avatar"
    And I wait for the page to be interactive
    And I click the avatar trigger for "gatin-avatar"
    Then the avatar trigger for "gatin-avatar" shows the stop label
    And the avatar "gatin-avatar" is in the "speaking" state
  ...
  Scenario: Clicking the trigger again stops the avatar
    ...
    And I click the avatar trigger for "gatin-avatar"
    And I click the avatar trigger for "gatin-avatar"
    Then the avatar "gatin-avatar" is in the "idle" state

  Scenario: X-ray identifies the Avatar component
    When I navigate to "/components/examples/avatar"
    And I wait for the page to be interactive
    And I hover over the avatar overlay "gatin-avatar"
