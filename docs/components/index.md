...
Feature: Component palette
  ...
  Scenario: from the future
    ...
    Then I can see first the Why (usage) through a meaningful simple use case
     ...
     And there are hidden features (except for feature.md) that 
       define the expected behaviour, and tag them by theme 
       (ui, spec, data, code, etc) and those tags are visible 
       in the automatic cards generated by the folder component
