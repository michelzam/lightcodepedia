...
- **"component ids are python compatible"** — enforces that every id is a valid Python identifier (letters, digits, underscores — no hyphens)
...
[Sample grid](#)
{: .datagrid bind="jss_data" id="jss_grid" rows="5" }

```python
@scenario("jss_data dataset is loaded and has 3 rows")
def check_dataset(self):
    ds = Dataset("jss_data")
    assert ds.loaded, "dataset not registered"
    assert ds.count == 3, f"expected 3 rows, got {ds.count}"

@scenario("jss_grid renders all rows")
def check_grid_rows(self):
    grid = self.page.jss_grid
    assert grid.exists,         "jss_grid not found — did you set id=\"jss_grid\"?"
    assert grid.row_count == 3, f"expected 3 rows, got {grid.row_count}"

@scenario("jss_grid headers are name and score")
...
def check_ds(self):
    ds = Dataset("my_data")        # accesses window.lcDatasets directly
    assert ds.loaded
    assert ds.count == 6
```
{: .jssteps }
...
- `self.page.<name>` resolves any component with `id="<name>"` on the page. Use Python identifiers (underscores, not hyphens): `id="jss_grid"` is accessible as `self.page.jss_grid`.
...
- Component ids must be valid Python identifiers — use underscores, not hyphens. The id is propagated to `data-lc-id` on the rendered wrapper.
