---
sitemap: false
layout: null
---

### ✍️ Text
Markdown basics — headings, lists, links, code, tables. Plus footnote popovers `[^x]` for inline definitions.

[Open →](text)

### 📑 Tabs
Alternative content panels — only one visible at a time. File-based, split by `###`.

[Open →](tabs)

### 🎠 Carousel
Auto-rotating items with clickable dots. Inline `items=` list.

[Open →](carousel)

### 📋 Dropdown
Click-to-reveal vertical menu. Inline `items=` pairs.

[Open →](dropdown)

### 🔘 Button
Styled link button with 5 color variants.

[Open →](button)

### 📻 Radio
Pick-one selector that reveals different content per choice.

[Open →](radio)

### 🪗 Accordion
Multiple collapsibles from one file, each independent.

[Open →](accordion)

### 📜 Scrollable
Fixed-height container with internal scrollbar.

[Open →](scrollable)

### 🪟 Embed page
Wrap another internal page in an iframe — perfect for safe nesting.

[Open →](embed_page)

### 🃏 Cards
Responsive grid of bordered cards with hover effect.

[Open →](cards)

### 📐 Grid
Free-form layout grid — cells can hold any content.

[Open →](grid)

### 💻 Code
Show YAML, Python, JSON, … with a title bar, or live-fetch from a real file in the repo.

[Open →](code)

### 🐍 Run
Live Python editor + runner via `{: .run }` — MicroPython compiled to WebAssembly, fully in-browser.

[Open →](run)

### 📊 Datagrid
Sortable, filterable, resizable tables via `{: .datagrid }` — AG Grid Community, YAML/JSON/CSV.

[Open →](datagrid)

### 📝 Form
Single-object attribute view via `{: .form }`. Pair with a datagrid via `bound=` for master/detail.

[Open →](form)

### 🗂️ Record
Schema-driven record editor via `{: .record }` — typed form, in-place WYSIWYG preview, lossless YAML round-trip, generic map, in-form AI and git commit.

[Open →](record)

### 🎬 Demo
Record a demonstration via `{: .demo }` — Start/Pause/Resume/Finish logs the learner's actions into one exportable file (summary + karma + sha-256). The black box to the recorder's camera; ▶ Replay re-performs the path through the avatar.

[Open →](demo)

### 🧑‍🏫 Avatar
A speaking character via `{: .avatar }` — it walks the page, spotlights the element it describes, and narrates (TTS, studio audio, or a recorded face). Guided tours, narrated videos, and the demo's replay engine.

[Open →](avatar)

### 📽️ Slides
Present any markdown page as a deck — `## h2` becomes a slide, bullets auto-fragment. No source markup; click 📽️ at the bottom-left to enter.

[Open →](slides)

### 🧪 Quiz
Turn a bullet list into an interactive question. Single-choice (immediate feedback) or multi-select (with Check button).

[Open →](quiz)

### 🤖 Agent
Chat-style LLM panel that calls GitHub Models from the browser. Each learner uses their own PAT — no backend, no shared key.

[Open →](agent)

### 🗒️ Pages
Your custom pages here.
 
[Open →](new2)

